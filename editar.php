<?php
// editar.php
// Formulario para editar un producto existente (UPDATE)
...
// Si no se proporcionó un ID válido, volver a la lista
if (!$id) {
...
// Si el producto no existe, volver a la lista
if (!$producto) {
    header('Location: productos.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($tipo) || $cantidad === '' || $costo === '' || empty($stock)) {
        ...
        $stmt = $pdo->prepare(
            "UPDATE productos SET nombre=?, tipo=?, cantidad=?, costo=?, stock=? WHERE id=?"
        );
        $stmt->execute([$nombre, $tipo, (int)$cantidad, (float)$costo, $stock, $id]);
...
<head>
    ...
    <title>Editar Producto – Grupo 9</title>
    <!-- Bootstrap 5 CSS -->
    ...
    <main class="flex-grow-1 p-4">
        ...
        <div class="mb-4">
            ...
            <p class="text-muted small">
                Modificando producto: <strong><?= htmlspecialchars($producto['nombre']) ?></strong>
                (ID: #<?= $id ?>)
            </p>
        </div>
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text"
                               name="tipo"
                               class="form-control"
                               value="<?= htmlspecialchars($producto['tipo']) ?>">
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-6">
                            ...
                            <input type="number"
                                   ...
                                   min="0"
                                   step="0.01"
                                   value="<?= htmlspecialchars($producto['costo']) ?>">
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="stock" class="form-select">
                            <option value="si" <?= $producto['stock'] === 'si' ? 'selected' : '' ?>>Sí</option>
                            <option value="no" <?= $producto['stock'] === 'no' ? 'selected' : '' ?>>No</option>
                        </select>
                    </div>
